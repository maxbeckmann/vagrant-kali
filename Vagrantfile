...
Vagrant.configure("2") do |config|
    ...
    config.vm.provision "shell" do |s|
      s.inline = <<-SHELL
        ...
        DEBIAN_FRONTEND=noninteractive apt-get upgrade -yq --ignore-missing
    ...
    config.vm.provision "shell" do |s|
      s.inline = <<-SHELL
        ...
        DEBIAN_FRONTEND=noninteractive apt install -y network-manager-openvpn-gnome openvpn-systemd-resolved
